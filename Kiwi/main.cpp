...
sf::Packet& operator>>(sf::Packet& packet, std::vector<std::string> v) {
    int size = 0;
    packet >> size;
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--downloadingMode") == 0 || strcmp(argv[1], "-d") == 0) {
            ...
            while (true) {
                ...
                switch (choice) {
                case '1': {
                    std::cout << "This mode is currently is not supported! Come back again!\n";
                    /*packet << "instrumentList\n";
                    socket.send(packet);
                    packet.clear();
                    socket.receive(packet);
                    std::cout << "Here's what's there to download:\n";
                    std::vector<std::string> instruments;
                    packet >> instruments;
                    for (int i = 0; i < instruments.size(); i++) {
                        std::cout << i + 1 << ") " << instruments[i] << '\n';
                    }
                    int num;
                    do {
                        std::cout << "\nEnter a number of a file: ";
                        std::cin >> num;
                    } while (num <= 0 || num >= instruments.size());

                    packet.clear();
                    packet << "downloadInstrument" << instruments[num - 1];
                    socket.send(packet);

                    std::ofstream file("Downloaded/" + instruments[num - 1], std::ofstream::binary);
                ...
                case '2': {
                    ...
                    bool response;
                    do {
                        std::cout << "Please, enter an ID of a project: ";
                        std::cin >> id;
                        packet.clear();
                        packet << "downloadKiwi" << id;
                        socket.send(packet);
                        packet.clear();
                        socket.receive(packet);
                        packet >> response;
                    ...
                    std::ofstream file("Downloaded/" + id, std::ofstream::binary);
                    receiveFile(&socket, file);
                    break;
                }
                    ...
                    if (noteClickedIndex >= 0) {
                        ...
                        // Если мы нажали на рабочую поверхность
                            ...
                            if (isNoteSet[noteAblsoluteIndex][cellAbsoluteIndex] && !isPlaying) {
                                double speed = std::pow(twelvesRootOf2, noteAblsoluteIndex - defaultInstrumentPitch);
